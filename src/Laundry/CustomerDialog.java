...
public class CustomerDialog extends JDialog {
    ...
    public CustomerDialog(Laundrify parent) {
        ...
        // Block digits from name field (letters, spaces, hyphens, apostrophes only)
        ((javax.swing.text.AbstractDocument) newNameField.getDocument()).setDocumentFilter(new javax.swing.text.DocumentFilter() {
            @Override
            public void insertString(javax.swing.text.DocumentFilter.FilterBypass fb, int off, String text, javax.swing.text.AttributeSet a) throws javax.swing.text.BadLocationException {
                if (text != null && !text.matches(".*\\d.*")) super.insertString(fb, off, text, a);
            }
            @Override
        ...
        // Allow only digits, max 11 characters for phone (DocumentFilter catches paste too)
        ((javax.swing.text.AbstractDocument) newPhoneField.getDocument()).setDocumentFilter(new javax.swing.text.DocumentFilter() {
            @Override
            public void insertString(javax.swing.text.DocumentFilter.FilterBypass fb, int off, String text, javax.swing.text.AttributeSet a) throws javax.swing.text.BadLocationException {
                String current = fb.getDocument().getText(0, fb.getDocument().getLength());
                if (text != null && text.matches("\\d*") && (current.length() + text.length()) <= 11)
            ...
            public void replace(javax.swing.text.DocumentFilter.FilterBypass fb, int off, int len, String text, javax.swing.text.AttributeSet a) throws javax.swing.text.BadLocationException {
                ...
                if (text == null || (text.matches("\\d*") && result.length() <= 11))
        ...
        btnRegisterNew.addActionListener(e -> {
            ...
            // Name must contain at least one letter and no digits
            if (!n.matches(".*[a-zA-Z].*") || n.matches(".*\\d.*")) {
                JOptionPane.showMessageDialog(this, "Name must contain letters only — no numbers.", "Invalid Name", JOptionPane.WARNING_MESSAGE);
                return;
            }
            // Phone must be exactly 11 digits starting with 09
            if (!p.matches("^09\\d{9}$")) {
                JOptionPane.showMessageDialog(this, "Phone must be 11 digits and start with 09\n(e.g. 09123456789).", "Invalid Phone", JOptionPane.WARNING_MESSAGE);
                return;
            }
            try (Connection conn = DatabaseConnection.getConnection();
