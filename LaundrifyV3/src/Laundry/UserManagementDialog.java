...
public class UserManagementDialog extends JDialog {
    ...
    private void resetUserPassword(JTable table, DefaultTableModel model) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            ps.setString(1, HashUtil.hashPassword(newPass));
            ps.setString(2, id);
            ps.executeUpdate();
    ...
    private void viewCustomerHistory() {
        ...
        // We use the ID to query the history perfectly now!
        String customerId = customerModel.getValueAt(row, 0).toString();
        ...
        historyDialog.setSize(750, 450); historyDialog.setLocationRelativeTo(this);
        historyDialog.setLayout(new BorderLayout(10, 10)); historyDialog.getContentPane().setBackground(BG_MAIN);
        ...
        // NORMALIZED: JOIN to get the list of items from the sales_details table
        String q = "SELECT s.invoice_number, GROUP_CONCAT(d.item_name SEPARATOR ', ') AS items_list, s.total_amount, s.status, s.sale_date " +
                   "FROM sales s LEFT JOIN sales_details d ON s.id = d.sale_id " +
                   "WHERE s.customer_id = ? GROUP BY s.id ORDER BY s.sale_date DESC";

        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(q)) {
            ...
            while (rs.next()) {
                historyModel.addRow(new Object[]{
                    ...
                    String.format("%.2f", rs.getDouble("total_amount")), rs.getString("status"), rs.getString("sale_date")
                });
            }
        ...
        bottomPanel.setOpaque(false); bottomPanel.add(btnClose);
    ...
    private void updateStrengthBar(String pw, JProgressBar bar, JLabel lbl) {
        ...
        if (pw.matches(".*[^a-zA-Z0-9].*") && score == 2) {
			score = 3;
		}
    ...
    private void styleTable(JTable table) {
        ...
        table.getTableHeader().setBackground(new Color(0, 70, 150));
