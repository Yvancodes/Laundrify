...
public class FileGeneration {
    ...
    public static void generateReceipt(String invoiceNumber, String customerName, String customerPhone,
        ...
        try {
            ...
            if (!customerPhone.isEmpty()) {
				document.add(new Paragraph("Contact: " + customerPhone));
			}
            document.add(new Paragraph(" "));
    ...
    public static double generateReportFromRS(ResultSet rs, String fileName, String title) throws Exception {
        ...
        Document document = new Document();
        // Set the document to Landscape so the longer text fits perfectly
        ...
        while (rs.next()) {
            ...
            // Extract the newly added Cashier Name
            String cashier = rs.getString("cashier_name");
            if(cashier == null || cashier.trim().isEmpty()) {
				cashier = "Unknown";
			}

            // Appended Cashier to the printed row
            document.add(new Paragraph(String.format(
                "Date: %s | Inv: %s | Cashier: %s | Cust: %s | Total: ₱%.2f | Paid: ₱%.2f | Change: ₱%.2f | Status: %s",
                rs.getString("sale_date"), rs.getString("invoice_number"), cashier, cust,
