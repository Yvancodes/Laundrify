...
public class AdminPanel extends JFrame {
    ...
    private final Color ACCENT_HOVER = new Color(225, 240, 255);  
    private final Color TEXT_MUTED = new Color(100, 120, 140);    
    private final Color DROPDOWN_BLUE = new Color(176, 196, 222); 
    ...
    public AdminPanel() {
        ...
        lblCashier.setForeground(TEXT_DARK);
        
        cashierFilterBox = new JComboBox<>();
        cashierFilterBox.setFont(new Font("Segoe UI", Font.BOLD, 12));
        cashierFilterBox.setBackground(DROPDOWN_BLUE); 
        cashierFilterBox.setForeground(TEXT_DARK);
        cashierFilterBox.setPreferredSize(new Dimension(130, 28));
        loadCashierDropdown(); 
        ...
        dateFilterBox.setBackground(DROPDOWN_BLUE); 
        dateFilterBox.setForeground(TEXT_DARK);
        dateFilterBox.setPreferredSize(new Dimension(130, 28));
        dateFilterBox.setSelectedIndex(1);
        dateFilterBox.addActionListener(e -> loadSalesData()); 
        
        filtersPanel.add(lblCashier);
        filtersPanel.add(cashierFilterBox);
        filtersPanel.add(lblDate);
        filtersPanel.add(dateFilterBox);

        summaryHeader.add(lblSummary, BorderLayout.WEST);
        summaryHeader.add(filtersPanel, BorderLayout.EAST);
        leftPanel.add(summaryHeader, BorderLayout.NORTH);

        tableModel = new DefaultTableModel(new String[]{"Inv.", "Customer", "Items", "Total", "Paid", "Change", "Status", "Cashier", "Date"}, 0) {
            public boolean isCellEditable(int row, int column) { return false; }
        };
        
        salesTable = new JTable(tableModel);
        salesTable.setFont(new Font("Segoe UI", Font.BOLD, 13)); 
        salesTable.setRowHeight(30);
        salesTable.setBackground(PRIMARY_BLUE);                  
        salesTable.setForeground(Color.WHITE);                   
        salesTable.setSelectionBackground(TEXT_DARK);            
        salesTable.setSelectionForeground(Color.WHITE);          
        salesTable.setGridColor(new Color(255, 255, 255, 80));   
        salesTable.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 13));
        salesTable.getTableHeader().setBackground(TEXT_DARK);    
        ...
        tableScroll.getViewport().setBackground(PRIMARY_BLUE);   
        ...
        btnRefresh.addActionListener(e -> { loadCashierDropdown(); loadSalesData(); });
        ...
        JButton btnAddons = new JButton("ADD-ONS SETUP");
        styleDynamicButton(btnAddons, CARD_WHITE, PRIMARY_BLUE); 
        btnAddons.addActionListener(e -> new AddonPricingSetup(this).setVisible(true)); 
        gridPanel.add(btnAddons);

        JButton btnUserMgmt = new JButton("USER");
        styleDynamicButton(btnUserMgmt, CARD_WHITE, PRIMARY_BLUE);
        btnUserMgmt.addActionListener(e -> new UserManagementDialog(this).setVisible(true));
        gridPanel.add(btnUserMgmt);
    ...
    private void loadSalesData() {
        ...
        String cashierFilter = cashierFilterBox.getSelectedItem() != null ? cashierFilterBox.getSelectedItem().toString() : "All Cashiers";
        ...
        if (dateFilter.equals("Today")) { queryBuilder.append("AND DATE(s.sale_date) = CURDATE() "); } 
        else if (dateFilter.equals("Last 7 Days")) { queryBuilder.append("AND s.sale_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) "); } 
        else if (dateFilter.equals("Last 30 Days")) { queryBuilder.append("AND s.sale_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) "); }
        
        if (!cashierFilter.equals("All Cashiers")) { queryBuilder.append("AND u.username = ? "); }
        
        queryBuilder.append("GROUP BY s.id ORDER BY s.sale_date DESC");
    ...
    private void searchSales() {
        ...
        String query = "SELECT s.invoice_number, COALESCE(c.name, 'Walk-in') AS customer_name, " +
                       "GROUP_CONCAT(CONCAT(d.item_name, ' (₱', d.price, ')') SEPARATOR ', ') AS items_list, " +
                       "s.total_amount, s.amount_paid, s.change_amount, s.status, COALESCE(u.username, 'Unknown') AS cashier_name, s.sale_date " +
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                tableModel.addRow(new Object[]{
                    ...
                    rs.getString("items_list") != null ? rs.getString("items_list") : "None", 
                    String.format("₱%.2f", rs.getDouble("total_amount")), String.format("₱%.2f", rs.getDouble("amount_paid")), 
                    String.format("₱%.2f", rs.getDouble("change_amount")), rs.getString("status"), 
                    rs.getString("cashier_name"), rs.getString("sale_date")
                });
                totalRevenue += rs.getDouble("total_amount");
            }
            totalSalesField.setText(String.format("₱%.2f", totalRevenue));
    ...
    private void printSalesReportByDate() {
        ...
        if (JOptionPane.showConfirmDialog(this, panel, "Print Sales Report", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION) {
            String start = startField.getText().trim(); String end = endField.getText().trim();
            String selectedCashier = printCashierBox.getSelectedItem().toString();

            String baseQuery = "SELECT s.invoice_number, COALESCE(c.name, 'Walk-in') AS customer_name, s.total_amount, s.amount_paid, s.change_amount, s.status, COALESCE(u.username, 'Unknown') AS cashier_name, s.sale_date " +
                               "FROM sales s LEFT JOIN customers c ON s.customer_id = c.id LEFT JOIN users u ON s.cashier_id = u.id ";

            if (selectedCashier.equals("All Cashiers")) {
                generatePDFReport(baseQuery + "WHERE DATE(s.sale_date) BETWEEN ? AND ? ORDER BY s.sale_date ASC", new String[]{start, end}, "sales/Sales_Report_" + start + "_to_" + end + ".pdf", "SALES REPORT: " + start + " TO " + end);
            } else {
                generatePDFReport(baseQuery + "WHERE u.username = ? AND DATE(s.sale_date) BETWEEN ? AND ? ORDER BY s.sale_date ASC", new String[]{selectedCashier, start, end}, "sales/" + selectedCashier.replaceAll("\\s+", "") + "_Report_" + start + "_to_" + end + ".pdf", "SALES REPORT (" + selectedCashier.toUpperCase() + "): " + start + " TO " + end);
            }
        }
    }
    ...
    private void backupAllSalesToPDF() {
        if (JOptionPane.showConfirmDialog(this, "Print a PDF of EVERY sale ever made?", "Backup Data", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            String q = "SELECT s.invoice_number, COALESCE(c.name, 'Walk-in') AS customer_name, s.total_amount, s.amount_paid, s.change_amount, s.status, COALESCE(u.username, 'Unknown') AS cashier_name, s.sale_date " +
                       "FROM sales s LEFT JOIN customers c ON s.customer_id = c.id LEFT JOIN users u ON s.cashier_id = u.id ORDER BY s.sale_date ASC";
            generatePDFReport(q, new String[]{}, "backup/Complete_Sales_Backup_" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + ".pdf", "COMPLETE DATABASE BACKUP");
        }
    }
    ...
    private void generatePDFReport(String query, String[] params, String fileName, String title) {
        ...
        } catch (Exception e) { JOptionPane.showMessageDialog(this, "Error creating PDF: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE); }
    }
    ...
    private void logout() {
        if (JOptionPane.showConfirmDialog(this, "Are you sure you want to log out?", "Confirm Logout", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            this.dispose(); new Login().getFrame().setVisible(true);
        }
    }
}