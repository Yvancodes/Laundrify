...
public class QueueManager extends JFrame {
    ...
    public QueueManager() {
        ...
        searchLabel.setForeground(new Color(15, 35, 65));
    ...
    private void styleBlueTable(JTable table, JScrollPane scroll) {
        ...
        table.getTableHeader().setBorder(BorderFactory.createMatteBorder(0, 0, 2, 0, PRIMARY_BLUE));

        scroll.setBorder(new LineBorder(BORDER_LIGHT, 1, true));
        scroll.getViewport().setBackground(Color.WHITE);
    }

    private void styleActionButton(JButton btn, Color accentColor) {
        ...
        btn.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent e) { btn.setBackground(accentColor.darker()); }
            public void mouseExited(java.awt.event.MouseEvent e)  { btn.setBackground(accentColor); }
        });
    }
    ...
    private void loadQueueData(String keyword) {
        pendingModel.setRowCount(0); unclaimedModel.setRowCount(0); claimedModel.setRowCount(0);

        String query = "SELECT s.id, s.invoice_number, COALESCE(c.name, 'Walk-in') AS customer_name, COALESCE(c.phone, '') AS customer_phone, s.status " +
                       "FROM sales s LEFT JOIN customers c ON s.customer_id = c.id ";
        boolean hasSearch = keyword != null && !keyword.isEmpty();

        if (hasSearch) { query += "WHERE c.name LIKE ? OR s.invoice_number LIKE ? "; }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (hasSearch) {
                ...
                pstmt.setString(1, searchPattern); pstmt.setString(2, searchPattern);
            }
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                String id = rs.getString("id"); String inv = rs.getString("invoice_number");
                String name = rs.getString("customer_name"); String phone = rs.getString("customer_phone");
